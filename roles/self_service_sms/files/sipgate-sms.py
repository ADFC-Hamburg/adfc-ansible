...
# create XMLRPC proxy
xmlrpc_url = "https://{username}:{password}@samurai.sipgate.net/RPC2".format(
    username=env["sms_username"], password=env["sms_password"]
)
...
if reply.get("StatusCode") == 200:
    ...
    print(
        "Error sending token to user {}. Sipgate returned: {}".format(
            env["selfservice_username"], reply
        )
    )
    sys.exit(1)
