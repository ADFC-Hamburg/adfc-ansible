...
from os import chmod, mkdir, listdir, replace, symlink, walk, makedirs
...
def get_valid_cert(cn):
    ...
    index_file.close()
    for line in lines[::-1]:
        (status, expire_date, revoce_date, serial,
         filename, cert_dn) = line.strip().split('\t')
...
def generate(user):
    ...
    subprocess.run(
        ['cp', f'//etc/univention/ssl/user/{zuid}/{zuid}.p12', zfile])
    subprocess.run(['chown', '-R', zuid, zhome])
    send_mail(user['mail'], 'Neues Zertikat',
              NEW_CERT_EMAIL % (zhome, password))
    ...
    for line in out.stdout.decode('utf-8').split('\n'):
        for keyword in keywords:
            if keyword in line:
                ...
                rtn[keyword.replace(':', '').strip()] = ":".join(
                    line_parts[1:])
    return rtn
...
def cron(user):
    ...
    if output != '':
        ...
        mailingliste = open(
            '/etc/ansible/secrets/mailingliste.txt').read().strip()
        send_mail(mailingliste,
                  'ADFC Zertifikate laufen demnächst ab', head+output)
...
def ldap_search_user(username):
    try:
        ...
        sys.exit(1)
    results = lo.search(filter='(uid=%s)' % username, attr=[
...
#        help="Renew certificate")
    ...
    group.add_argument('--cron',
                       dest='action', action='store_const', const=cron,
                       help="Do cron things.")
